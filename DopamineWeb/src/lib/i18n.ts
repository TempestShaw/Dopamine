...
const en = {
  ...
  errors: {
    ...
    forget: "Couldn't forget that. If this keeps happening, update the Dopamine app.",
  },
  footer: {
  ...
  rules: {
    open: (title: string) => `Choose a category for “${title}”`,
    contains: "Windows whose title contains",
    countsAs: "count as",
    everyBrowser: "in every browser",
    inApp: (app: string) => `in ${app}`,
    remove: "remove this rule",
    browserHint: "Browsers are sorted by the site in each window. Click a window to choose its category.",
    appHint: "Click a window to give just that window a category.",
  },
  forget: {
    title: (title: string) => `Forget “${title}” in this period`,
    session: "Forget this session",
    ask: "Erase for good? The title is deleted and the time stops counting. This can't be undone.",
    yes: "forget",
    no: "keep",
  },
  share: {
...
const zhCN: Dict = {
  ...
  errors: {
    ...
    forget: "没能删除这条记录。如果一直这样，请更新 Dopamine 应用。",
  },
  footer: {
    local: "所有数据都只留在这台电脑上",
  ...
  rules: {
    open: (title) => `为“${title}”选择分类`,
    contains: "标题包含",
    countsAs: "的窗口算作",
    everyBrowser: "适用于所有浏览器",
    inApp: (app) => `仅限 ${app}`,
    remove: "删除这条规则",
    browserHint: "浏览器按每个窗口里的网站分类。点一个窗口可以单独设置它的分类。",
    appHint: "点一个窗口可以只给这个窗口设置分类。",
  },
  forget: {
    title: (title) => `删除这段时间里的“${title}”`,
    session: "删除这个时段",
    ask: "永久删除？标题会被抹掉，时间也不再计入。无法撤销。",
    yes: "删除",
    no: "保留",
  },
  share: {
...
const zhTW: Dict = {
  ...
  errors: {
    ...
    forget: "沒能刪除這筆紀錄。如果一直這樣，請更新 Dopamine 應用程式。",
  },
  footer: {
    local: "所有資料都只留在這台電腦上",
  ...
  rules: {
    open: (title) => `為「${title}」選擇分類`,
    contains: "標題包含",
    countsAs: "的視窗算作",
    everyBrowser: "適用於所有瀏覽器",
    inApp: (app) => `僅限 ${app}`,
    remove: "刪除這條規則",
    browserHint: "瀏覽器依每個視窗裡的網站分類。點一個視窗可以單獨設定它的分類。",
    appHint: "點一個視窗可以只為這個視窗設定分類。",
  },
  forget: {
    title: (title) => `刪除這段時間裡的「${title}」`,
    session: "刪除這個時段",
