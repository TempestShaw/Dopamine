...
public sealed class StoredSettings
{
    ...
    /// <summary>Windows the user sorted by hand, keyed by cleaned title. They also train the dashboard's title model.</summary>
    public Dictionary<string, string> TitleLabels { get; set; } = new();
    ...
    public void Apply(SettingsPatch patch)
    {
        ...
        if (patch.TitleLabels != null)
            TitleLabels = patch.TitleLabels.Where(p => p.Key.Length is > 0 and <= 300 && Categories.Contains(p.Value)).Take(3000).ToDictionary(p => p.Key, p => p.Value);
